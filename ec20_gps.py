...
class PostInfo(object):
    ...
    def csqStatus(self,Csq):
        self.csq = Csq
        url = "http://127.0.0.1:8080/api/status-info"
        data = {"color":"green","label":"信号","value":self.csq}
        headers = {'content-type': 'application/json','charset': 'utf-8'}
        res = requests.post(url,data=json.dumps(data),headers=headers)
...
def ec20CSQ(ser):
     at_csq = 'AT+CSQ\r\n'
     ser.write(at_csq.encode())
     csq=ser.read(1)
     csq = re.findall(r'\d+',re.sub("\r|\n","",ser.read(ser.inWaiting()).decode(encoding="utf8",errors="ignore")))
     return csq
...
ser_csq = serial.Serial('/dev/ttyUSB3',9600)

while True:
        dataList = re.sub('\r|\n','',ser_gps.readline().decode(encoding="utf8",errors="ignore")).split(',')
        #print(dataList)
        if '$GNRMC' in dataList and dataList[2] == "A":
            ...
            post = PostInfo()
            post.speedStatus(speed)
            post.GPS(lng,lat)
            #print(dataList)
        
        csqData=ec20CSQ(ser_csq)
        if len(csqData) > 0:
            csqData[0] = str(int(csqData[0]) *2 - 113) + "db"
            post = PostInfo()
            post.csqStatus(csqData[0])
            #print(csqData[0])
